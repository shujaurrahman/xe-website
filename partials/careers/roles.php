...
<section class="band band--alt car-roles" id="roles" aria-labelledby="roles-t">
  <div class="wrap">
    <div class="bdh-head bdh-head--row" data-rv>
      ...
      <div><p class="lead">Search, filter, open a role for the detail, and apply right there. No account, no cover-letter portal.</p></div>
    </div>
    ...
    <div class="car-list" data-car-list>
      ...
      <details class="car-role" id="role-<?= e($car_id) ?>" data-dept="<?= e($car_r['dept']) ?>" data-loc="<?= e($car_r['loc']) ?>" data-type="<?= e($car_r['type']) ?>" data-text="<?= e($car_text($car_r)) ?>"<?= $car_ok ? '' : ' hidden' ?><?= $car_open === $car_id ? ' open' : '' ?>>
        ...
        <div class="car-role__body">
          ...
          <form class="car-apply" action="<?= e(xe_url('contact.php')) ?>" method="post" aria-label="Apply for <?= e($car_r['title']) ?>">
            ...
            <div class="car-hp" aria-hidden="true"><label for="car-<?= e($car_id) ?>-w">Leave this empty</label><input type="text" id="car-<?= e($car_id) ?>-w" name="website" tabindex="-1" autocomplete="off"></div>
            ...
            <label class="car-f"><span>Your application <i>add a portfolio or LinkedIn link</i></span><textarea name="message" rows="5" maxlength="4000" required>Application: <?= e($car_r['title']) ?> (<?= e($car_id) ?>)&#10;&#10;Portfolio or LinkedIn: &#10;&#10;Why this role: </textarea></label>
            <button class="btn btn--ink car-apply__go" type="submit">Send application <span class="i" aria-hidden="true">›</span></button>
            <p class="car-apply__note">Goes to our hiring team through the contact form. We reply to every application.</p>
          </form>
        </div>
      </details>
      <?php endforeach; ?>
