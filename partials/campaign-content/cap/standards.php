  ...
  <div class="wrap">
    <div class="bdh-head bdh-head--row" data-rv>
      <div><p class="lbl lbl--blue"><span class="dot"></span>Frameworks we build to</p>
        <h2 class="h2" id="standards-t"><?= ccd_h('std', 'The rules,', 'designed in from the brief.') ?></h2></div>
      <div><p class="lead"><?= ccd_lead('std', 'Standards this work is designed and checked against. They are frameworks we build to, not certifications we hold.') ?></p></div>
    </div>
    <ul class="ccd-std__g">
      ...
      <li class="ccd-std__i" data-rv>
        ...
        <p class="sm"><?= e($CCD_T['apply'][$ccd_k] ?? $ccd_s['apply'] ?? $ccd_s['covers'] ?? '') ?></p>
      </li>
      <?php endforeach; ?>
    </ul>
  </div>
