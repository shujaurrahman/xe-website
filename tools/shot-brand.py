    ...
    for path in paths:
        ...
        if FULL:
            ...
            sels = [SEL] if SEL else pg.evaluate("""()=>[...document.querySelectorAll('main > section, main > nav')].map((s,i)=>{
                const c=[...s.classList].find(c=>/^(bd|bg|bi|bf|bs|ba|bt|xt|tih|twa|tcs|tas|tap|tic|tsc|tis|tsv|taa|ttw|cch|ccd|aih|aid|pxh|pxd|mth|mtd|ind|wrk|apr|car|lgl|svx|ct|e404)-[a-z0-9-]+$/.test(c)||/^s\d\d$/.test(c)); return c?'.'+c:null}).filter(Boolean)""")
            seen = {}
            for sel in sels:
